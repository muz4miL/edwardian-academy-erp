...
export const ViewEditTeacherModal = ({
    ...
    useEffect(() => {
        if (teacher && open) {
            ...
            setJoiningDate(teacher.joiningDate ? teacher.joiningDate.split('T')[0] : "");
    ...
    const handleSave = () => {
        ...
        if (compType === "percentage") {
            ...
            compensation.profitShare = Number(profitShare);
        }
                ...
                <div className="grid gap-6 py-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={subject} onValueChange={setSubject} disabled={isReadOnly}>
                    ...
                    <div className="space-y-4 bg-secondary/30 p-4 rounded-xl border border-border">
                        ...
                        <RadioGroup
                            value={compType}
                            onValueChange={(value) => !isReadOnly && setCompType(value as CompensationType)}
                            className="grid grid-cols-1 md:grid-cols-3 gap-3"
                            disabled={isReadOnly}
                        >
                            <div className={`flex items-center space-x-2 border border-border rounded-lg p-3 ${isReadOnly ? 'opacity-70' : 'cursor-pointer hover:border-primary/50'} transition-colors bg-card`}>
                                <RadioGroupItem value="percentage" id="r1" className="text-primary" disabled={isReadOnly} />
                                <Label htmlFor="r1" className={`font-normal w-full ${!isReadOnly && 'cursor-pointer'}`}>
                                    Percentage
                                </Label>
                            </div>
                            <div className={`flex items-center space-x-2 border border-border rounded-lg p-3 ${isReadOnly ? 'opacity-70' : 'cursor-pointer hover:border-primary/50'} transition-colors bg-card`}>
                                <RadioGroupItem value="fixed" id="r2" className="text-primary" disabled={isReadOnly} />
                                <Label htmlFor="r2" className={`font-normal w-full ${!isReadOnly && 'cursor-pointer'}`}>
                                    Fixed Salary
                                </Label>
                            </div>
                            <div className={`flex items-center space-x-2 border border-border rounded-lg p-3 ${isReadOnly ? 'opacity-70' : 'cursor-pointer hover:border-primary/50'} transition-colors bg-card`}>
                                <RadioGroupItem value="hybrid" id="r3" className="text-primary" disabled={isReadOnly} />
                                <Label htmlFor="r3" className={`font-normal w-full ${!isReadOnly && 'cursor-pointer'}`}>
                                    Hybrid
                                </Label>
                            </div>
                        </RadioGroup>
                        ...
                        <div className="grid gap-4 mt-4">
                            ...
                            {compType === "hybrid" && (
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-sm text-muted-foreground">Profit Share (%)</Label>
                                        <Input
                                            type="number"
                                            value={profitShare}
                ...
                <DialogFooter>
                    {mode === "view" ? (
                        ...
                        <>
                            <Button
                                variant="outline"
                                onClick={() => mode === "edit" && teacher ? setMode("view") : onOpenChange(false)}
